...
def main():
    ...
    print("  ⚠️ TWO QUANTITIES, NOT ONE. The LEVEL of the identical-mask rate says whether the mask")
    print("     is stable AT ALL; the SPREAD across rho says whether TEXTURE is what decides it.")
    print("     Reading only the spread mislabels a noise-unstable mask as texture-sensitive.")
    print("  %-14s %-6s %-28s %-7s %s"
          % ("cut", "sigma", "identical masks, rho -0.8..+0.9", "level", "spread"))
    for k, sig, rates, spread in ms:
        print("  BLANK+%-8.2f %-6.1f %-28s %5.0f%%   %5.1f pts"
              % (k, sig, " ".join("%3.0f%%" % (100 * r) for r in rates),
                 100 * sum(rates) / len(rates), 100 * spread))
    print()
    ...
    print("     ⚠️ AND THE DEPENDENCE IS NON-MONOTONIC -- it PEAKS near 2 sigma. Three regimes:")
    print("       <=1 sigma  the mask is unstable at EVERY texture (level 0-1%), so the verdict")
