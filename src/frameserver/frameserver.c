...
static void process_item(frameserver *f, const fs_item *it){
    if(it->gap_only){
        ...
        if(f->log){
            ...
            if (wr >= 0) wr = log_field(f->log, NULL);
            if (wr >= 0) wr = fprintf(f->log, ",Uncalibrated,n.a.,-128,0,0.000,0.000,0.000,%d,%lld,0,0,RingFullTail,%u,%llu,0,0,0,NoRaster\n",
    ...
    if (f->log){
        ...
        if (wr >= 0) wr = fprintf(f->log, ",%s,%s,%d,%d,%.3f,%.3f,%.6f,%d,%lld,%d,%d,%s,%u,%llu,%d,%d,%d,%s\n",
