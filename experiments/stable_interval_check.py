...
S, bottom (top+239 clipped), the unit-to-unit changes of each, and the unmeasurable units. Exit 1 if any measurable
...
for f in ('1','2'):
    ...
    print(f'  bottom {dict(collections.Counter(bots).most_common())}')
    ...
    print(f'  measurable-to-measurable changes: top {len(ct)} {ct[:10]} | S {len(cs)} {cs[:10]} | bottom {len(cb)} {cb[:10]}')
