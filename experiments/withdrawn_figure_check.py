...
def report(bare, mentions) -> int:
    ...
    print("  ⚠️ AND THE DEFECT IT CANNOT SEE IS PRODUCED BY A RULE BOTH AGENTS FOLLOW CORRECTLY.")
    print("     A reviewer who declines to rewrite another agent's table -- which is the right call")
    print("     on a file it does not own -- HAS NOWHERE TO PUT THE CAVEAT EXCEPT BELOW IT. So the")
    print("     claim-above/withdrawal-below shape is generated wherever two authors touch one")
    print("     entry, and only the ORIGINAL author can repair it, being the only one permitted to.")
    print("     Verified on the one real instance: the table row and the qualification beneath it")
    print("     are different commits by different agents. That is a structural consequence of the")
    print("     ownership convention, not a lapse by either side, and it will recur.")
