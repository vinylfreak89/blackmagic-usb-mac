...
| A18 | The engine's only lock-acquisition site requires `measurement->switch_measurable` unconditionally (`field_registration.c:833`), so a boxed source — whose head switch rule 8 forbids measuring — can never acquire a lock. A lock needs two or more observations, one of which must be geometry | contract rule 4 | not fixed; **tier 1**, blocks every other capture-1 item |
