...
| # | item | where it is written down | state |
|---|---|---|---|
| B1 | The four references, one commit each, in the acceptance order | contract §8; `experiments/geometry_oracle/REFERENCE_SPEC.md` states every column's raw-row derivation | **capture 1's reference is built and its invariant CHECKED: 0 violations** (`stable_interval_check.py`, 508 units from counter 6667, both fields measurable in all 508). Captures 2, 3 and 4 not started — they follow capture 1's acceptance |
