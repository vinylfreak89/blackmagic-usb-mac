...
struct frameserver {
    ...
    int8_t last_published_d1, last_published_d2;
...
static void process_item(frameserver *f, const fs_item *it){
    if(it->gap_only){
        ...
        if(f->log){
            ...
            if (wr >= 0) wr = log_field(f->log, NULL);
            if (wr >= 0) wr = fprintf(f->log, ",Uncalibrated,n.a.,-128,0,0.000,0.000,0.000,%d,%lld,0,0,RingFullTail,%u,%llu,0,0,0,NoRaster,0,0,Unknown,0,0,0,0\n",
    ...
    if (unit){
        ...
        int rc = fp_publish(f->pub, unit, FP_UNIT_BYTES, obs.counter_extended,
                            have_d ? d.applied_d1 : f->last_published_d1,
                            have_d ? d.applied_d2 : f->last_published_d2,
                            obs.transport == UNIT_TRANSPORT_COMPLETE ? FP_TRANSPORT_COMPLETE : FP_TRANSPORT_SHORT,
                            aknown, apts);
        if (rc == 0){
            f->st.published++; published = 1;
            if (have_d) {
                f->last_published_d1 = d.applied_d1;
                f->last_published_d2 = d.applied_d2;
            }
        } else {
            if (rc == 1) f->st.publisher_dropped++;
            /* An unpublished crop must not become the next comb witness. */
            fieldreg_discontinuity(f->eng);
    ...
    if (f->log){
        ...
        if (wr >= 0) wr = fprintf(f->log, ",%s,%s,%d,%d,%.3f,%.3f,%.6f,%d,%lld,%d,%d,%s,%u,%llu,%d,%d,%d,%s,%d,%u,%s,%.6f,%.6f,%.6f,%.6f,%d,%u\n",
            ...
            have_d ? d.comb_unresolved_alternatives : 0);
