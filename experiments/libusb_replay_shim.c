...
int libusb_handle_events_timeout(libusb_context *ctx, struct timeval *tv){
    ...
    int delivered=0;
    static int burst=0;
    if(!burst){ const char*b=getenv("REPLAY_BURST"); burst=b?atoi(b):4; if(burst<1)burst=1; }
    while(delivered<burst){
        ...
        s->fill++;
        { static int pace=-1;
          if(pace<0){ const char*pv=getenv("REPLAY_PACE_US"); pace=pv?atoi(pv):0; }
          if(pace>0 && s->fill>=x->num_iso_packets) usleep(pace); }
