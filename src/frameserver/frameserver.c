...
static int log_field(FILE *L, const fieldreg_field_decision *d)
{
    ...
    return fprintf(L,
                   ",%s,%s,%d,%s,%s,%u,%u,%d,%d,%d,%d,%s,%.3f,%d,%.3f"
                   ",%d,%d,%.3f"
                   ",%d,%d,%d,%d,%d,%d,%d,%d"
                   ",%d,%d,%d,%d,%d"
                   ...
                   d && d->gap_row >= 0 ? d->gap_row + 4 : -1,
                   d ? d->gap_d : FIELDREG_UNKNOWN,
                   d && d->gap_measurable,
...
static void process_item(frameserver *f, const fs_item *it){
    if(it->gap_only){
        ...
        if(f->log){
            ...
            if (wr >= 0) wr = log_field(f->log, NULL);
            if (wr >= 0) wr = fprintf(f->log, ",Uncalibrated,Uncalibrated,0,n.a.,-128,0.000,0.000,0.000000,n.a.,-128,0,0.000,0.000,0.000000,%d,%lld,0,0,RingFullTail,%u,%llu\n",
    ...
    if (f->log){
        ...
        if (wr >= 0) wr = fprintf(f->log, ",%s,%s,%u,%s,%d,%.3f,%.3f,%.6f,%s,%d,%d,%.3f,%.3f,%.6f,%d,%lld,%d,%d,%s,%u,%llu\n",
            have_d ? fieldreg_parity_state_name(d.parity_state) : "Uncalibrated",
            have_d ? fieldreg_gap_state_name(d.gap_state) : "Uncalibrated",
            have_d ? d.gap_agreement_count : 0,
