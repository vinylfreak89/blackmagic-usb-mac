...
3. **Acceptance, capture 1, then 2, 3, 4, in that order** (harness runs, both agents read): the engine's record vs the
   ...
   **A signal-state change is an engine change, and the classification is not stored anywhere.** The classifier
   feeds registration live inside the frameserver (parser → signal_state → engine, per unit), so there is no saved
   classification to patch: the only persisted forms are the decision-log sidecars a replay or a recording writes
   and the renders made from them. Any change to `src/signal_state/` therefore invalidates every sidecar and every
   render made before it, and they are regenerated by re-running the replay over each capture in the acceptance
   order — including `captures/fulltape_render_registration.csv` and its render when the whole tape is next run
   (owner, 2026-09-09: "it needs to run through the tpc again and reclassify and rewrite that wherever its written
   (or maybe its live feed to registration...)" — it is the live feed). The harness's own references are measured
   from the raw rows and do not depend on the classifier; what depends on it is the engine's expected behaviour on
   the units rule 5 gates.
