...
struct frameserver {
    ...
    fs_stats st; _Atomic uint64_t audio_records, audio_resync, dropped_pool_full, video_obs, holes, unframed, shorts, other_fmt, ns0800;
...
static int take_slot(frameserver *f){
    ...
    if (used + 1 > f->st.pool_high_water) f->st.pool_high_water = used + 1;
...
static void push(frameserver *f, const fs_item *it){
    ...
    if (h - t >= RING_ITEMS){ atomic_fetch_add(&f->dropped_pool_full, 1); if (it->slot >= 0) atomic_store(&f->slot_used[it->slot], 0); return; }
...
static void on_video(void *ctx, const unit_video_observation *u){
    ...
    fs_item it; it.slot = -1; it.obs = *u; it.obs.bytes = NULL; it.obs.payload = NULL;
    if (u->fixed_raster_eligible && u->byte_count == UNIT_PARSER_VIDEO_UNIT_BYTES){
        int s = take_slot(f);
        if (s < 0){ atomic_fetch_add(&f->dropped_pool_full, 1); return; }
...
static void log_header(FILE *L){
    fprintf(L, "ordinal,counter_extended,transport,kind,appearance,appearance_confidence,source,source_confidence,"
               "interval_id,unsettled,provisional_d1,provisional_d2,applied_d1,applied_d2,committed_d1,committed_d2,"
               "settled_known,settled_d1,settled_d2,resolution,evidence_mode,confidence,published\n");
}
static void process_item(frameserver *f, const fs_item *it){
    ...
    if (f->log){
        fprintf(f->log, "%llu,%llu,%s,%d,%s,%.3f,%s,%.3f,%llu,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s,%.3f,%d\n",
    ...
    if (it->slot >= 0) atomic_store(&f->slot_used[it->slot], 0);
}
static void *worker_main(void *arg){
    ...
    for (;;){
        ...
        if (h == t){
            if (atomic_load(&f->producer_done)) break;
...
int fs_open(frameserver **out, const fs_config *cfg){
    ...
    f->cfg = *cfg;
    f->n_slots = cfg->pool_units ? cfg->pool_units : 16;
