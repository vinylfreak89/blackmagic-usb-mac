...
static void check_field(int k) {
    int off=263*k;ge_bottom_evidence e;double margin;
    memset(y,10,sizeof y);margin=3;
    assert(bottom_scan(y,k,1,&e,margin)==0 && e.rule==GE_BOTTOM_UNKNOWN);
    assert(e.measured && e.p5==10 && e.p50==10 && e.p95==10);
    body(257+off,13); /* uniform picture: median difference exactly 3 */
    assert(bottom_scan(y,k,1,&e,margin)==261+off && e.rule==GE_BOTTOM_FLAT_REFERENCE);
    body(257+off,10);memset(y+(257+off)*720+40,13,64); /* partial high side */
    assert(bottom_scan(y,k,1,&e,margin)==261+off);
    body(257+off,10);memset(y+(257+off)*720+40,7,64); /* low side */
    assert(bottom_scan(y,k,1,&e,margin)==261+off);
    body(257+off,10);body(237+off,13);
    assert(bottom_scan(y,k,1,&e,margin)==241+off); /* inclusive scan limit */
    body(237+off,10);body(236+off,100);
    assert(bottom_scan(y,k,1,&e,margin)==0); /* outside scan */
    body(257+off,13);margin=3+5e-10;
    assert(bottom_scan(y,k,1,&e,margin)==261+off); /* representation tolerance */
    margin=3+2e-9;
    assert(bottom_scan(y,k,1,&e,margin)==0);
    body(257+off,10);y[(257+off)*720+40+607]=13;
    memset(y+(257+off)*720+40+608,14,32);margin=3.05;
    assert(bottom_scan(y,k,1,&e,margin)==261+off); /* interpolated 13.05 minus 10 */
    margin=3;
    ...
    assert(bottom_scan(y,k,1,&e,margin)==(k?525:263) && e.rule==GE_BOTTOM_FALLBACK);
    body(258+off,10);body(257+off,13);
    assert(bottom_scan(y,k,1,&e,margin)==261+off); /* qualified F never judges row below it */
    memset(y,10,sizeof y);body(257+off,20);
    memset(y+(258+off)*720+40,14,64);
    assert(bottom_scan(y,k,1,&e,margin)==261+off && e.p95-e.p5==4);
    memset(y+(258+off)*720+40,15,64);
    assert(bottom_scan(y,k,1,&e,margin)==262+off && e.rule==GE_BOTTOM_FALLBACK);
}
int main(void){
