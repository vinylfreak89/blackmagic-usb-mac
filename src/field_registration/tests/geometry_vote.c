...
static void paired_raster(void) {
    for(unsigned i=0;i<GE_PIXELS;i++)y[i]=i%2?41:1;
}
...
static void votes(void) {
    geometry_engine g;ge_init(&g,0,0);paired_raster();
...
static void fills(void) {
    ...
    ge_level_result v=ge_level_scan(y,0,5);
    assert(v.first==23 && v.reference==1 && v.mean==22 && v.sd==10 && v.corr_below==1 && v.accepted);
    assert(ge_level_scan(y,0,0).accepted); /* top 23 remains within clamp zero */
    memset(y+20*720+40,1,640);
    v=ge_level_scan(y,0,5);assert(v.first==23 && v.corr_below==0 && !v.accepted); /* sd==10 */
    memset(y+19*720+40,12,640);v=ge_level_scan(y,0,5);assert(!v.accepted && v.sd==0);
    assert(!ge_level_scan(y,0,5).accepted); /* flat off */
    memset(y,1,sizeof y);
    for(int r=25;r<=26;r++)for(int x=40;x<680;x++)y[r*720+x]=20+(x%2)*20;
    v=ge_level_scan(y,0,5);assert(v.first==29 && !v.accepted);
    assert(ge_level_scan(y,0,6).accepted);
...
static void ownership(void) {
    geometry_engine g;ge_init(&g,1,0);paired_raster();
    ...
    top.wave_status[0]=GE_WAVE_ABSTAIN;top.level[0].accepted=0;
...
static void paired_tops(void) {
    ...
    geometry_engine g;ge_init(&g,1,0);
