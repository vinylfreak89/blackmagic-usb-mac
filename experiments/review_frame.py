...
def build_frame(raw, c, capture_name, fields, extra_header=()):
    """The review frame for ONE unit: 720x486 as placed beside the 525-line raster, box overlaid.

    Factored out of main() so the whole-capture render and the single-unit stills draw from ONE
    builder. Two renderers would be the two-stores defect, and the burned-in record is precisely
    the thing that must not drift from what produced it. `fields` is {1:{...},2:{...}} of display
    values (T, S, top, clip, and an optional `extra` line); each caller adapts its own sidecar into
    that, so the renderer never learns a schema. With no `extra` and no `extra_header` the output
    is byte-identical to the pre-refactor stills, GATED on that by SHA-256 rather than assumed.
    """
    Y=raw[:,1::2]
    Y=raw[:,1::2]
    rgb=uyvy_to_rgb(raw)
    ...
    for f in (1,2):
        ...
        txt="field %d:  T=%s  S=%s  top=%s  clip=%s   box=%s"%(
            f, e.get('T','?'), e.get('S','?'), e.get('top','?'), e.get('clip','?'), vd)
        ...
        if e.get('extra'):
            d.text((28,line),e['extra'],(200,200,208)); line+=14
    for t in extra_header:
        d.text((10,line),t,(240,240,246)); line+=14
    return img
...
def main():
    ...
    for c in sorted(got):
        img=build_frame(got[c], c, os.path.basename(a.capture),
                        {f: ({k: eng[(c,f)][k] for k in ('T','S','top','clip')}
                             if (c,f) in eng else {}) for f in (1,2)})
