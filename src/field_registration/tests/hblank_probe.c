...
static void frames(FILE *f,const ge_decision *out,unsigned n) {
    for(unsigned i=0;i<n;i++)if(out[i].has_frame) {
        ...
        fprintf(f,"%llu,%llu,%d,%d,%d,%d,%.17g,%d,%u,%d,%d,%d,%d,%d,%s,%s,%d,%d,%d,%d\n",
            ...
            d->top_overrun_veto[0],d->top_overrun_veto[1]);
    }
}
int main(int argc,char **argv) {
    ...
    while(fread(&counter,sizeof counter,1,stdin)==1) {
        ...
        printf("%d,%d,%d,%.17g,%.9f,%.9f,%d,%d,%.17g,%.17g,%d,%d,%s,%s,%d,%d\n",
            ...
            current->top_overrun_veto[0],current->top_overrun_veto[1]);
    }
