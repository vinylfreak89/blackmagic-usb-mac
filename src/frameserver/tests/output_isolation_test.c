...
static int publish_test(fp_publisher *p,const uint8_t *u,size_t n,uint64_t ctr,int d1,int d2,uint8_t tr,int ak,uint64_t pts){
    ...
    if(mode==0 && k==1){atomic_store(&blocked,1);while(!atomic_load(&released))usleep(100);}
...
void fs_test_after_log_row(frameserver *f,FILE *l){
    (void)f;(void)l;
    if(mode==1 && !atomic_exchange(&blocked,1))while(!atomic_load(&released))usleep(100);
}
static void ended(void *p,enum cc_end e){(void)p;(void)e;atomic_store(&done,1);}
int main(int argc,char **argv){
    assert(argc==3);alarm(40);mode=atoi(argv[2]);
    ...
    if(mode!=2){
        ...
        for(int i=0;i<5000 && atomic_load(&analyzed)<120;++i)usleep(1000);
        pass=atomic_load(&analyzed)==120;
        fprintf(stderr,"%s_does_not_block_analysis: %s (%u/120 analyzed while blocked)\n",
                mode==0?"publisher":"logger",pass?"PASS":"FAIL",atomic_load(&analyzed));
