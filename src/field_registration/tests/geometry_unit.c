...
int main(void) {
    ...
    for(int d=-5;d<=5;d++) {
        ...
        ge_comb_result c=ge_comb(a,b,NULL);
        assert(c.shift==d && c.decided && isinf(c.margin));
    }
    memset(a,1,GE_PIXELS);ge_features f;ge_measure(a,&f,NULL);
    assert(!f.first[0]&&!f.first[1]&&!f.last[0]&&!f.last[1]&&!f.bottom[0]&&!f.bottom[1]);
    ge_comb_result c=ge_comb(a,a,NULL);assert(!c.decided && c.margin==1);
    ge_decision out[2];ge_init(g,0,0,NULL);
    ...
    ge_init(g,1,0,NULL);assert(ge_push(g,a,20,1,out)==0);
    ...
    memset(a,1,GE_PIXELS);ge_measure(a,&f,NULL);
    assert(f.hblank_cols[0]==24 && f.hblank_level[0]==1);
    for(int r=18;r<=261;r++)a[r*720+1]=3;
    ge_measure(a,&f,NULL);assert(f.hblank_cols[0]==1 && f.hblank_level[0]==1);
